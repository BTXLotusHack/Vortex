...
function extractJobPayload(payload: any): ManusScrapeResult {
  ...
  const techStack = normalizeTechStack(
    source?.techStack || source?.tech_stack || source?.skills || source?.stack
  );
...
export async function scrapeLinkedinJob(jobUrl: string): Promise<ManusScrapeResult> {
  if (!MANUS_BASE_URL) {
