...
async function getPendingSignup(email: string) {
  const result = await query<PendingSignupRow>(
    `
...
export async function startSignupVerification(input: {
  ...
  const existingUser = await query<{ password_hash: string | null }>(
    `
...
export async function resendSignupVerificationOtp(email: string) {
