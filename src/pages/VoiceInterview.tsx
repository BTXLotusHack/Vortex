...
import { ArrowLeft, ArrowRight, CheckCircle2, FileText, Mic, MicOff, Sparkles } from "lucide-react";
...
export default function VoiceInterview() {
  ...
  const fetchConversationToken = async (): Promise<string | null> => {
    ...
    for (const endpoint of candidates) {
      try {
        const response = await fetch(`${API_URL}${endpoint}?agentId=${encodeURIComponent(agentId)}`, {
          method: "GET",
        });
  ...
  const startVolumeLoop = () => {
    ...
    const tick = async () => {
      ...
      const next = Math.max(0.18, Math.min(1, inputLevel * 0.9 + outputLevel * 0.9 + 0.18));
  ...
  const startConversation = async () => {
    try {
      ...
      const sharedHandlers = {
        ...
        onMessage: ({ role, message }: { role: "user" | "agent"; message: string }) => {
      ...
      try {
        ...
        console.warn("WebRTC conversation start failed, retrying with websocket.", primaryError);
        conversation = await Conversation.startSession({
      ...
      console.error(error);
      toast.error("Could not start the ElevenLabs interview. Check microphone permissions and agent configuration.");
    }
  };
  ...
  const markInterviewComplete = () => {
    updatePipeline({
      ...
      recommendedNextStep: pipeline.technicalRequired ? "technical" : "complete",
    });
        ...
        {!interviewsUnlocked ? (
          <div className="rounded-[2rem] border border-luxe bg-card p-8">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-muted-foreground">
              This stage only unlocks after the candidate CV has been analyzed against the job description. That fit assessment is what Alex uses to interview the candidate properly.
            </p>
            ...
            >
              ...
              <div className="relative max-w-3xl">
                ...
                <p className="mt-5 max-w-2xl text-base leading-8 text-muted-foreground">
                  The agent receives the CV summary, the job description, and the CV-to-JD fit analysis before the conversation starts. The left side is the live interview surface and the right side is the running dialogue transcript.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
              <div className="surface-glass rounded-[2rem] border border-luxe p-6">
                <div className="mb-5 flex items-center justify-between gap-3">
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.26em] text-muted-foreground">Live Interview</p>
                ...
                <div className="flex min-h-[640px] flex-col items-center justify-center rounded-[1.75rem] border border-luxe bg-[radial-gradient(circle_at_top,hsl(164_48%_88%/0.28),transparent_38%),linear-gradient(180deg,hsl(38_55%_97%),hsl(145_27%_95%))] p-8">
                  <div className="relative flex h-[320px] w-[320px] items-center justify-center">
                    ...
                    <div
                      className={cn(
                        ...
                        status === "connected" ? "h-[250px] w-[250px]" : "h-[220px] w-[220px]"
                      )}
                  ...
                  <div className="mt-8 grid w-full gap-3 sm:grid-cols-3">
                    <div className="rounded-2xl bg-white/60 px-4 py-4">
                      ...
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">{currentJobRole}</p>
                    </div>
                    <div className="rounded-2xl bg-white/60 px-4 py-4">
                      <p className="text-sm font-medium">JD fit</p>
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        {(candidateProfile?.jobFitScore ?? 0) > 0 ? `${candidateProfile?.jobFitScore}/100` : "Estimated"}{" "}
                        {candidateProfile?.jobFitVerdict ? `- ${candidateProfile.jobFitVerdict}` : ""}
                      </p>
                    </div>
                    <div className="rounded-2xl bg-white/60 px-4 py-4">
              ...
              <div className="space-y-6">
                <div className="surface-glass rounded-[2rem] border border-luxe p-6">
                  <p className="text-[11px] uppercase tracking-[0.26em] text-muted-foreground">Dialogue</p>
                  <h2 className="mt-2 text-xl font-semibold">Interview transcript</h2>
                  <div className="mt-5 flex max-h-[520px] min-h-[520px] flex-col gap-3 overflow-y-auto rounded-[1.5rem] border border-luxe bg-card p-4">
                    {messages.length === 0 ? (
                      <div className="flex h-full flex-col items-center justify-center text-center text-sm leading-7 text-muted-foreground">
                        ...
                        Start the session and the transcript will appear here as Alex and the candidate speak.
                      </div>
                    ) : (
                ...
                <div className="surface-glass rounded-[2rem] border border-luxe p-6">
                  <p className="text-[11px] uppercase tracking-[0.26em] text-muted-foreground">Interview context</p>
                  <div className="mt-4 space-y-4">
                    <div className="rounded-2xl bg-card px-4 py-4">
                      ...
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        {candidateProfile?.summary || "No CV summary available."}
                      </p>
                    </div>
                    <div className="rounded-2xl bg-card px-4 py-4">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        <Sparkles className="h-4 w-4 text-primary" /> Signals for follow-up
                      </div>
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        Strengths: {candidateProfile?.strengths?.join(", ") || "N/A"}
                      </p>
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        Risks: {candidateProfile?.risks?.join(", ") || "N/A"}
                      </p>
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        Likely skills: {candidateProfile?.likelySkills?.join(", ") || "N/A"}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="surface-glass rounded-[2rem] border border-luxe p-6">
                  <p className="text-[11px] uppercase tracking-[0.26em] text-muted-foreground">Next step</p>
                  <h2 className="mt-2 text-xl font-semibold">Move the pipeline forward</h2>
                  <p className="mt-2 text-sm leading-7 text-muted-foreground">
                    Once the interview is done, mark it complete here and continue to the technical round if needed.
                  </p>
                  ...
                  <div className="mt-5 space-y-3">
                    ...
                    >
                      ...
                      {markedComplete ? "Voice interview marked complete" : "Mark Voice Interview Complete"}
                    </button>

                    {fromPipeline && pipeline.technicalRequired && (
                      <button
                        onClick={() => navigate("/technical-interview?from=pipeline")}
                      ...
                      >
                        Continue to Technical Interview <ArrowRight className="h-4 w-4" />
                      </button>
                    )}
